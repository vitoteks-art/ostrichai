...
const WEBHOOK_URL = 'https://n8n.vitotek.com.ng/webhook/main-ads';
...
const callWebhook = async (step: string, payload: any) => {
  try {
    // Enrich payload: Map 'name' to 'productName' if missing, as requested
...
export const fetchCompetitorAds = async (product: ProductInput): Promise<any[]> => {
  // We use the same webhook flow, treating this as the raw data fetch for Step 1
...
export const analyzeMarket = async (product: ProductInput, competitorAds: any[] = []): Promise<MarketIntelligence> => {
  // 1. WEBHOOK DATA MAPPING
  // If we have data passed in (from App.tsx calling fetchCompetitorAds) OR we call it here.
  let webhookData = competitorAds.length > 0 ? competitorAds[0] : null;

  // If not provided, try calling specifically for analysis
  if (!webhookData) {
    webhookData = await callWebhook('market_intel', product);
  }

  // Handle wrapped response if it comes nested (e.g. from fetchCompetitorAds returning [data])
  // It might be directly the data object if callWebhook unwrapped it
  ...
  if (data && (data.painPoints || data.productAnalysis)) {
      ...
      // Construct MarketIntelligence object directly from webhook data
...
export const buildMessaging = async (product: ProductInput, intelligence: MarketIntelligence): Promise<MessagingArchitecture> => {
  ...
  // Handle the specific "output" wrapper format from the webhook response
  ...
  // Handle flat response if format changes
...
export const generateCreatives = async (messaging: MessagingArchitecture, intelligence?: MarketIntelligence | null): Promise<AdCreative[]> => {
  ...
  // Handling for user's specific response structure for Step 3
  if (webhookResponse) {
    let concepts = [];
    ...
    if (concepts.length > 0) {
      return concepts.map((concept: any, index: number) => {
        // Handle new response format: concept.type (Image/Video) or concept.format
        ...
        return {
          ...
          // Use productionNotes or script for video scripts
...
export const predictPerformance = async (creatives: AdCreative[], audience: string): Promise<Record<string, PredictionResult>> => {
  ...
  if (webhookResponse) {
    ...
    // Check for specific "output.evaluations" structure from webhook
    ...
    if (evaluations.length > 0) {
      ...
      evaluations.forEach((evalItem: any) => {
        // Map webhook ID (e.g., "concept-1") to ad ID.
        const adId = evalItem.creativeId || evalItem.adId;

        // Normalize 1-10 scale to 0-100 if necessary
  ...
  // 2. Fallback
  // We process ads in a batch for comparison context
  const prompt = `
  ...
  // Using Pro for deeper reasoning capabilities
  const response = await ai.models.generateContent({
...
export const generateTargeting = async (product: ProductInput, intelligence: MarketIntelligence): Promise<AudienceTargeting> => {
  ...
  // Handle flat/legacy webhook response
...
export const generateOptimizationInsights = async (
  ...
  if (webhookResponse && webhookResponse.output) {
    ...
    return {
      ...
      // Fallback mappings for older UI components
      metrics: {
      ...
      insights: out.optimizationActions?.map((act: any) => ({
        ...
        impact: 'HIGH' // Default since new format prioritizes differently
      })) || []
    };
  }
  ...
  const prompt = `
    ...
    1. GENERATE REALISTIC SIMULATED PERFORMANCE DATA for this 7-day period.
       - Calculate Spend, Impressions, Clicks, CTR (Average is 1-2%), CPA (Cost Per Acquisition), and ROAS.
    2. Based on these specific simulated numbers, identify fatigue or opportunities.
  ...
  const schema: Schema = {
    ...
    properties: {
      metrics: {
        type: Type.OBJECT,
        properties: {
          spend: { type: Type.STRING, description: "e.g., '$1,250'" },
          impressions: { type: Type.STRING, description: "e.g., '45,000'" },
          clicks: { type: Type.STRING, description: "e.g., '850'" },
          ctr: { type: Type.STRING, description: "e.g., '1.85%'" },
          cpa: { type: Type.STRING, description: "e.g., '$24.50'" },
          roas: { type: Type.STRING, description: "e.g., '2.4x'" }
        }
      },
      insights: {
        type: Type.ARRAY,
        items: {
          type: Type.OBJECT,
          properties: {
            action: { type: Type.STRING, description: "Specific action like 'Increase budget' or 'Kill Ad Set'" },
            type: { type: Type.STRING, enum: ['BUDGET', 'CREATIVE', 'AUDIENCE'] },
            reasoning: { type: Type.STRING, description: "Why this action is recommended based on the simulated metrics (cite the metrics!)" },
